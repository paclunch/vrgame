...
public class Tally8 : MonoBehaviour {
	...
	void Update () {
		if (ritzBitz.selected == 1 && stringCheese.selected == 1 && oreos.selected == 1 && sandwich.selected == 1) {
			SceneManager.LoadScene ("Scene9");	
			finished = true;
		}
		if (flintstonesThermos.selected + scoobyDooThermos.selected + babyCarrots.selected + macAndCheese.selected + chipsAhoy.selected + goldfish.selected + orange.selected + lunchables.selected + candy.selected + soup.selected + milkCarton.selected + dinosaurChickenNuggets.selected + chips.selected + juiceBox.selected + mickeyFriendsThermos.selected + powerRangersThermos.selected + disneyPrincessThermos.selected + helloKittyThermos.selected + lizzieMcGuireThermos.selected + peanutsThermos.selected + apple.selected + brownie.selected + grapes.selected == 3) {
