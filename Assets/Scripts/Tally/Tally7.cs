...
public class Tally7 : MonoBehaviour {
	...
	void Update () {
		if (grapes.selected == 1 && goldfish.selected == 1 && dinosaurChickenNuggets.selected == 1 && juiceBox.selected == 1) {
			SceneManager.LoadScene ("Scene8");	
			finished = true;
		}
		if (flintstonesThermos.selected + scoobyDooThermos.selected + babyCarrots.selected + macAndCheese.selected + chipsAhoy.selected + ritzBitz.selected + oreos.selected + orange.selected + lunchables.selected + candy.selected + sandwich.selected + soup.selected + milkCarton.selected + stringCheese.selected + chips.selected + mickeyFriendsThermos.selected + powerRangersThermos.selected + disneyPrincessThermos.selected + helloKittyThermos.selected + lizzieMcGuireThermos.selected + peanutsThermos.selected + apple.selected + brownie.selected == 3) {
