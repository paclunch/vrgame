...
public class Tally16 : MonoBehaviour {
	...
	void Update () {
		if (chips.selected == 1 && milkCarton.selected == 1 && stringCheese.selected == 1 && apple.selected == 1 && peanutsThermos.selected == 1) {
			SceneManager.LoadScene ("Scene17");	
			finished = true;
		}
		if (flintstonesThermos.selected + scoobyDooThermos.selected + babyCarrots.selected + macAndCheese.selected + chipsAhoy.selected + ritzBitz.selected + oreos.selected + goldfish.selected + orange.selected + lunchables.selected + candy.selected + sandwich.selected + soup.selected + dinosaurChickenNuggets.selected + juiceBox.selected + mickeyFriendsThermos.selected + powerRangersThermos.selected + disneyPrincessThermos.selected + helloKittyThermos.selected + lizzieMcGuireThermos.selected + brownie.selected + grapes.selected == 3) {
