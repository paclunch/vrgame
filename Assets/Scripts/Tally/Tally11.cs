...
public class Tally11 : MonoBehaviour {
	...
	void Update () {
		if (helloKittyThermos.selected == 1 && milkCarton.selected == 1 && stringCheese.selected == 1 && candy.selected == 1 && apple.selected == 1) {
			SceneManager.LoadScene ("Scene12");	
			finished = true;
		}
		if (flintstonesThermos.selected + scoobyDooThermos.selected + babyCarrots.selected + macAndCheese.selected + chipsAhoy.selected + ritzBitz.selected + oreos.selected + goldfish.selected + orange.selected + lunchables.selected + sandwich.selected + soup.selected + dinosaurChickenNuggets.selected + chips.selected + juiceBox.selected + mickeyFriendsThermos.selected + powerRangersThermos.selected + disneyPrincessThermos.selected + lizzieMcGuireThermos.selected + peanutsThermos.selected + brownie.selected + grapes.selected == 3) {
