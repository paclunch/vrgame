...
public class Tally14 : MonoBehaviour {
	...
	void Update () {
		if (ritzBitz.selected == 1 && orange.selected == 1 && sandwich.selected == 1 && juiceBox.selected == 1 && oreos.selected == 1 && soup.selected == 1) {
			SceneManager.LoadScene ("Scene15");	
			finished = true;
		}
		if (flintstonesThermos.selected + scoobyDooThermos.selected + babyCarrots.selected + macAndCheese.selected + chipsAhoy.selected + goldfish.selected + lunchables.selected + candy.selected + milkCarton.selected + dinosaurChickenNuggets.selected + stringCheese.selected + chips.selected + juiceBox.selected + mickeyFriendsThermos.selected + powerRangersThermos.selected + disneyPrincessThermos.selected + helloKittyThermos.selected + lizzieMcGuireThermos.selected + peanutsThermos.selected + apple.selected + brownie.selected + grapes.selected == 3) {
