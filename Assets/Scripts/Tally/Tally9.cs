...
public class Tally9 : MonoBehaviour {
	...
	void Update () {
		if (lizzieMcGuireThermos.selected == 1 && soup.selected == 1 && juiceBox.selected == 1 && chipsAhoy.selected == 1) {
			SceneManager.LoadScene ("Scene10");	
			finished = true;
		}
		if (flintstonesThermos.selected + scoobyDooThermos.selected + babyCarrots.selected + macAndCheese.selected + ritzBitz.selected + oreos.selected + goldfish.selected + orange.selected + lunchables.selected + candy.selected + sandwich.selected + milkCarton.selected + dinosaurChickenNuggets.selected + stringCheese.selected + chips.selected + mickeyFriendsThermos.selected + powerRangersThermos.selected + disneyPrincessThermos.selected + helloKittyThermos.selected + peanutsThermos.selected + apple.selected + brownie.selected + grapes.selected == 3) {
