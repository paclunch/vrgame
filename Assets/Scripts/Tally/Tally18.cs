...
public class Tally18 : MonoBehaviour {
	...
	void Update () {
		if (mickeyFriendsThermos.selected == 1 && babyCarrots.selected == 1 && orange.selected == 1 && soup.selected == 1 && candy.selected == 1 && milkCarton.selected == 1) {
			SceneManager.LoadScene ("Scene19");	
			finished = true;
		}
		if (flintstonesThermos.selected + scoobyDooThermos.selected + macAndCheese.selected + chipsAhoy.selected + ritzBitz.selected + oreos.selected + goldfish.selected + lunchables.selected + sandwich.selected + dinosaurChickenNuggets.selected + stringCheese.selected + chips.selected + juiceBox.selected + powerRangersThermos.selected + disneyPrincessThermos.selected + helloKittyThermos.selected + lizzieMcGuireThermos.selected + peanutsThermos.selected + apple.selected + brownie.selected + grapes.selected == 3) {
