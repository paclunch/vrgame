...
public class Tally17 : MonoBehaviour {
	...
	void Update () {
		if (flintstonesThermos.selected == 1 && juiceBox.selected == 1 && goldfish.selected == 1 && grapes.selected == 1 && brownie.selected == 1) {
			SceneManager.LoadScene ("Scene18");	
			finished = true;
		}
		if (scoobyDooThermos.selected + babyCarrots.selected + macAndCheese.selected + chipsAhoy.selected + ritzBitz.selected + oreos.selected + orange.selected + lunchables.selected + candy.selected + sandwich.selected + soup.selected + milkCarton.selected + dinosaurChickenNuggets.selected + stringCheese.selected + chips.selected + mickeyFriendsThermos.selected + powerRangersThermos.selected + disneyPrincessThermos.selected + helloKittyThermos.selected + lizzieMcGuireThermos.selected + peanutsThermos.selected + apple.selected == 3) {
